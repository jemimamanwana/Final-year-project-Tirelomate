...
async function handleServiceBooking(serviceId) {
    ...
    if (!rankedProviders.length) {
        try {
            const _bk = await window.bookings.create({ service_id: serviceId, provider_id: service.provider_id, date, time, notes, total_price: service.price });
            console.log('Booking response:', _bk);
            _showPaymentStep(_bk.id, service.title, service.provider?.name || 'Provider', service.price);
            return;
        } catch (error) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        try {
            booking = await window.bookings.create({
                ...
                time,
                notes,
                total_price: candidate.price || service.price
            });
        } catch (err) {
...
function handleQuickBook(e) {
    ...
    (async () => {
        try {
            ...
            await window.bookings.create({ service_id: selected.id, provider_id: selected.provider_id, date, time, notes: details, total_price: selected.price });
    ...
    if (window.auth && window.auth.isLoggedIn()) {
        try {
            await window.bookings.create({ service_id: selectedService.id, provider_id: selectedService.provider_id, date, time, notes: details, total_price: selectedService.price });
        } catch (_) {}
    }
