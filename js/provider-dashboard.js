...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(async () => {
        ...
        // Check user type — redirect non-providers to customer dashboard
        try {
            const currentUser = await window.auth.getCurrentUser();
            if (currentUser && currentUser.user_type !== 'provider') {
                if (!window.location.pathname.includes('Services-provider-dashboard')) {
                    window.location.href = 'dashboard.html';
                    return;
                }
            }
        } catch (e) {
            console.warn('Could not check user role, continuing:', e.message);
        }
...
async function loadServices() {
    ...
    try {
        ...
        showNotification('Could not load services. Check your connection.', 'error');
    }
...
async function loadBookings() {
    ...
    try {
        ...
        showNotification('Could not load bookings.', 'error');
    }
...
async function loadEarnings() {
    ...
    try {
        ...
        showNotification('Could not load earnings data.', 'error');
...
async function loadProviderReviews() {
    ...
    try {
        ...
        showNotification('Could not load reviews.', 'error');
    }
}
